...
namespace dp2Mini
{
    internal class PrintCardDocument : PrintDocument
    {
        ...
        internal void DoPrintPage(IWin32Window owner,
            string strStyle,
            ...
            out string strError)
        {
            ...
            for (int i = 0; i < page.Lines.Count; i++)
            {
                ...
                Font font = BuildFont(line.FontDef);
            ...
            if (bIsHeaderFooter == false)
            {
                ...
                for (int i = 0; i < nodes.Count; i++)
                {
                    ...
                    for (int j = rect_group.Paragraphs.Count - 1; j >= 0; j--)
                    {
                        ...
                        {
                            ResetLines(pages, rect, p_group.Lines);
                        }
                        else
                            break;  // 只要中间不连续，就中断
                    }
            ...
            out string strError)
        {
            ...
            for (int i = 0; i < node.ChildNodes.Count; i++)
            {
                ...
                if (child.NodeType == XmlNodeType.Text)
                {
                    ...
                    if (macro_table != null)
                    {
                        strText = MacroString(macro_table, strText);
                    }
            ...
            out Color color)
        {
            ...
            return BuildFont(strFontString);
        }
            ...
            PrintLine line)
        {
            ...
            // 如果是中文字符
            if (IsHanzi(strCurrentChar[0]) == true)
                return -1;
    ...
    public delegate void SetProgressEventHandler(object sender, SetProgressEventArgs e);
